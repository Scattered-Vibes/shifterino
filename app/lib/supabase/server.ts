...
export function createClient() {
  ...
  // Use getAll, setAll, and a custom remove implementation based on setAll
  return createServerClient<Database>(
    process.env.NEXT_PUBLIC_SUPABASE_URL!,
    process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!,
    {
      cookies: {
        // Use getAll to retrieve cookies (returns all cookie objects)
        get: cookieStore.getAll,
        // Use setAll so Supabase can update cookies properly
        set: cookieStore.setAll,
        // Remove cookies by setting them with an empty value and a maxAge of 0
        remove: (name: string) => {
          cookieStore.setAll([{ name, value: '', maxAge: 0 }])
        }
      },
      auth: {
