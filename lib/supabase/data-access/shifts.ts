...
import type { Shift, ShiftUpdateData } from '@/types/shift'
...
export const shiftQueries = {
  ...
  async createShift(shift: Omit<Shift, 'id' | 'created_at' | 'updated_at'>) {
    const supabase = createClient()
    try {
      const { data, error } = await supabase
        .from('individual_shifts')
  ...
  async updateShift(shiftId: string, updateData: ShiftUpdateData) {
    const supabase = createClient()
    try {
      const { data, error } = await supabase
        .from('individual_shifts')
